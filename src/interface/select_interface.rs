...
pub fn select_interface(
    ...
    if docker_mode {
        info!(
            "Docker Modeが有効な為、{}インターフェイスで自動実行されます。",
            docker_interface_name
        );
        return if let Some(interface) = interfaces
            .iter()
            .find(|interface| interface.name == docker_interface_name)
        {
            Ok(interface.clone())
        } else {
            Err(InterfaceError::DockerInterfaceNotFound(
                docker_interface_name.to_string(),
            ))?
        };
    }
    ...
    io::stdout()
        .flush()
        .map_err(|e| InterfaceError::StdoutFlushError(e.to_string()))?;

    let mut input = String::new();
    io::stdin()
        .read_line(&mut input)
        .map_err(|e| InterfaceError::ReadLineError(e.to_string()))?;
